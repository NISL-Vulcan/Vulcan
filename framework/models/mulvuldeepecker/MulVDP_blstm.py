...
class MulVDP_BLSTM(nn.Module):
    ...
    def __init__(self, config, vocabulary):
        super(MulVDP_BLSTM, self).__init__()
        self._config = config
    ...
    def init_layers(self):
        ...
        # Dropout layers
        self.g_dropout_rnn = nn.Dropout(self._config.global_encoder.rnn_dropout)
        self.l_dropout_rnn = nn.Dropout(self._config.global_encoder.rnn_dropout)

        # Hidden layers
        layers = [
            nn.Linear(self._config.classifier.hidden_size, self._config.classifier.hidden_size),
    ...
    def txt_rnn_embedding(self, txt: torch.Tensor, words_per_label: List[int], op: str):
    ...
    def forward(self, gadgets: torch.Tensor, global_words_per_label: List[int], attns: torch.Tensor, local_words_per_label: List[int]) -> torch.Tensor:
        """
