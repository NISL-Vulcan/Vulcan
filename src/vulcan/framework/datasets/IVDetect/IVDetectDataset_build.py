...
def IVD_dataset_build(glove_flag=False):
    data_file = "/root/astBugDetection/framework/datasets/IVDetect/data.csv"
...
def generate_glove_file(data):
    ...
    for key, data in sample_big_data.items():
        with open(os.path.dirname(__file__)+f'/glove/{key}_word.txt', 'w') as file:
...
def load_glove_vectors(filename):
    """Loads glove vectors."""
    glove_file_dir = os.path.join(os.path.dirname(__file__), f'glove/{filename}_vectors.txt')
    tmp_file_dir = os.path.join(os.path.dirname(__file__), f'glove/{filename}_gensim.txt')
    temp_file = get_tmpfile(tmp_file_dir)
    if not os.path.isfile(tmp_file_dir):
        glove2word2vec(glove_file_dir, temp_file)
    return KeyedVectors.load_word2vec_format(temp_file)

def IVD_dataset_build(glove_flag=False):
    data = pd.read_csv("/root/astBugDetection/framework/datasets/IVDetect/data.csv")
    ...
    for i in tqdm(range(len(data))):
        ...
        if 5 <= loc <= 500:
            ...
            torch.save(graph_i, os.path.join(os.path.dirname(__file__), f"data/pyg_graph/data_{i}.pt"))
