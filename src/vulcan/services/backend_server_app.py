...
def run_validation(job: TrainingJob):
    ...
    try:
        ...
        # 构建验证命令 - 使用模块入口（避免依赖 tools/ 目录）
        val_script = None
        config_path = os.path.abspath(job.config_path)
        
        job.add_log("✅ 将使用 python -m vulcan.cli.val 执行验证")
        print("✅ 将使用 python -m vulcan.cli.val 执行验证")
...
def test_validation_setup():
    ...
    try:
        ...
        if not test_results["validation_script_exists"]:
            test_results["issues"].append("未找到验证入口（建议使用 console script: vulcan-val）")
            test_results["recommendations"].append("请确认已安装项目并可运行: vulcan-val --cfg <path>")
        else:
...
def run_training(job: TrainingJob):
    ...
    try:
        ...
        job.add_log("✅ 训练入口: python -m vulcan.cli.train")
        print("✅ 训练入口: python -m vulcan.cli.train")
...
def run_validation_in_training(job: TrainingJob, project_root: str):
    ...
    try:
        ...
        job.add_log("✅ 验证入口: python -m vulcan.cli.val")
        print("✅ 验证入口: python -m vulcan.cli.val")
